...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const [userResult, questsResult, skillsResult] = await Promise.all([
      supabase.from('users').select('username, level, xp, streak').eq('id', req.userId).single(),
      supabase.from('quests').select('title, domain, completed, xp_reward, quest_date')
        .eq('user_id', req.userId)
        .order('created_at', { ascending: false })
        .limit(12),
      supabase.from('skills').select('domain, xp').eq('user_id', req.userId).order('xp', { ascending: false }).limit(10),
    ]);
