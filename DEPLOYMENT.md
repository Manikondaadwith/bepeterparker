...
4. **Add Environment Variables** (in Render dashboard → Environment):
   ...
   | Variable | Value |
   |----------|-------|
   | `JWT_SECRET` | Any random long string (e.g. `myspiderverse2024secretXYZ123!`) |
   | `CORS_ORIGINS` | `*` |
   | `NODE_ENV` | `production` |
...
- Add env vars: `JWT_SECRET`, `CORS_ORIGINS=https://your-frontend.vercel.app`
...
`better-sqlite3` needs to compile native code. On Render, this works automatically since it's a Linux environment with build tools. If you hit issues, add this to your Render build command:
```
apt-get update && apt-get install -y python3 make g++ && npm run install:all && npm run build
```
